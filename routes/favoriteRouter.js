...
.get(cors.cors, authenticate.verifyUser, (req, res, next) => {
    ...
    .populate('user') // .ref not needed!
    .populate('campsites') // same
    .then(favorite => { // favorite singular
        res.statusCode = 200;
...
.post(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then(favorite => {
        if (favorite) {
            ...
            favorite.save() // this actually saves to mongodb
            .then(favorite => {
                res.statusCode = 200;
                res.setHeader('Content-Type', 'application/json');
                res.json(favorite);
...
.delete(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    .then(favorite => {
        if (favorite) {
            ...
            if (index >= 0) { // returns -1 if there's nothing there
...
module.exports = favoriteRouter; // don't forget to export so we can use this elsewhere!